...
public class KafkaAdminClient {
    ...
    private void checkMaxRetry(int retry, Integer maxRetry) {
        if (retry > maxRetry) throw new KafkaClientException("Reached the max number of retry for reading kafka topic(s)");
    }
    ...
    private void sleep(Long sleepTimeMs){
        try {
            ...
            throw new KafkaClientException("Errow while sleeping for waiting new thread to be created!");
        }
    }
    ...
    private Collection<TopicListing>  doGetTopics(RetryContext context) throws ExecutionException, InterruptedException {
        LOG.info("Reading kafka topic {}, attempt {}", kafkaConfigData.getTopicNamesToCreate(), context.getRetryCount());
